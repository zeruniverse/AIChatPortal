...
async function load() { try { data.value = await api(`/api/public/shares/${route.params.token}`, { auth: false }); error.value = ''; } catch (e) { error.value = e.message; } finally { loading.value = false; } }
function connect() { events = openEventStream(`/api/public/shares/${route.params.token}/events`, { update: load, unavailable: () => { error.value = '分享链接已关闭或对话已删除'; data.value = null; }, error: () => { events?.close(); clearInterval(poll); poll = setInterval(load, 5000); } }, { auth: false }); }
      ...
      <section class="turn-list">
        <article v-for="turn in data.turns" :key="turn.turnNo" class="turn-card">
          <section class="message user-message">
            ...
            <div v-if="turn.hasAttachments" class="attachment-row"><span>本轮附件：{{ turn.attachmentReady ? formatSize(turn.attachmentSize) : '正在处理' }}</span><a v-if="turn.attachmentReady" class="button secondary compact" :href="apiUrl(`/api/public/shares/${route.params.token}/turns/${turn.turnNo}/attachments`)">下载附件</a></div>
          </section>
