...
<template>
  <main class="page conversation-page public-page">
    ...
    <template v-if="data">
      <header class="page-header"><div><p class="eyebrow">公开分享</p><h1>{{ data.title }}</h1><p class="muted">拥有此链接的人可以查看内容和下载每轮附件。</p></div></header>
      <section class="turn-list">
        <article v-for="turn in data.turns" :key="turn.turnNo" class="turn-card">
          <section class="message user-message">
            ...
            <div v-if="turn.hasAttachments" class="attachment-row"><span>本轮附件：{{ turn.attachmentReady ? formatSize(turn.attachmentSize) : '正在压缩' }}</span><a v-if="turn.attachmentReady" class="button secondary compact" :href="`/api/public/shares/${route.params.token}/turns/${turn.turnNo}/attachments`">下载本轮附件包</a></div>
          </section>
