...
export function parseAnswerSegments(raw) {
  ...
  while (cursor < source.length) {
    ...
    if (!close) {
      segments.push({ type: 'think', content: source.slice(contentStart).trim(), complete: false, index: thinkIndex++ });
      cursor = source.length;
      break;
    }
    segments.push({ type: 'think', content: source.slice(contentStart, close.index).trim(), complete: true, index: thinkIndex++ });
